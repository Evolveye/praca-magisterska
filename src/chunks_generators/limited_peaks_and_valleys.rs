...
pub struct GeneratorOfLimitedPeaksAndValleys {
    noise: SimplexNoise,
    noise_frequency: f64,
    noise_amplitude: f64,
}

impl GeneratorOfLimitedPeaksAndValleys {
    #[allow(unused)]
...
impl WorldGenerative for GeneratorOfLimitedPeaksAndValleys {
    fn generate_chunk( &self, dataset:&mut VoxelDataset, origin:(i64, i64, i64), size:u8 ) -> Octree<Voxel> {
        ...
        if origin.0 > 2 || origin.1 > 2 || origin.2 > 2 {
            return world_holder
        }
        ...
        let grass_level = 30 - world_origin.1;
        let quadtree = Quadtree::from_terrain_generation( max_depth, &|x, z| {
            self.noise.noise3d(
                (world_origin.0 + z as i64) as f64 * self.noise_frequency,
                1.0,
                (world_origin.2 + x as i64) as f64 * self.noise_frequency,
            )
        } );
